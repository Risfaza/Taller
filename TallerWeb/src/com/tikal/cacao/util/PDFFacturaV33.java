...
public class PDFFacturaV33 {
	...
	private Font fontHeadConceptos = new Font(Font.FontFamily.HELVETICA, 5.5F, Font.NORMAL);
	private Font fontConceptos = new Font(Font.FontFamily.HELVETICA, 5.5F, Font.NORMAL);
	...
	public void resetPieDePagina(){
		pieDePagina = new MyFooter(null);
	}
	...
	public Document construirPdf(String descripcionUsoDeCFDI, String descripcionRegimenFiscal, String descripcionFormaDePago,
			String descripcionTipoCFDI){
		this.descripcionUsoDeCFDI = descripcionUsoDeCFDI;
		this.descripcionRegimenFiscal = descripcionRegimenFiscal;
		this.descripcionFormaDePago = descripcionFormaDePago;
	...
	public Document construirPdf(Comprobante comprobante, String selloDigital, byte[] bytesQRCode, Imagen imagen,
			Estatus estatus, String comentarios) throws MalformedURLException, DocumentException, IOException {
	...
	private void construirBoceto(Comprobante comprobante, Imagen imagen, Estatus estatus, TimbreFiscalDigital tfd,
			String comentarios) throws MalformedURLException, DocumentException, IOException {
	...
	private void construirUsoCFDIYDatosFiscales(Comprobante comprobante, Estatus estatus, TimbreFiscalDigital tfd)
		...
		Chunk chunkUsoCFDI = new Chunk(
				((comprobante.getReceptor().getUsoCFDI()!=null)?comprobante.getReceptor().getUsoCFDI().getValor():"") + " " + this.descripcionUsoDeCFDI, font3);
...
//		tablaConceptos.setWidths(new float[] { 9,7, 6, 6, 35, 10, 10, 8 });
...
//		agregarCeldaConFondo("No. Identificación", fontHeadConceptos, tablaConceptos, true);
		...
		agregarCeldaConFondo("Cant.", fontHeadConceptos, tablaConceptos, true);
		...
		agregarCeldaConFondo("IVA", fontHeadConceptos, tablaConceptos, true);

		List<Concepto> listaConceptos = comprobante.getConceptos().getConcepto();
		for (Concepto concepto : listaConceptos) {
			agregarCelda((concepto.getClaveProdServ()!=null)?concepto.getClaveProdServ().getValor():"", fontConceptos, tablaConceptos, true);
//			agregarCelda(concepto.getNoIdentificacion(), fontConceptos, tablaConceptos, true);
			agregarCelda((concepto.getCantidad()!=null)?concepto.getCantidad().toString():"", fontConceptos, tablaConceptos, true);
			agregarCelda((concepto.getClaveUnidad()!=null)?concepto.getClaveUnidad().getValor():"", fontConceptos, tablaConceptos, true);
			agregarCelda((concepto.getUnidad()!=null)?concepto.getUnidad():"", fontConceptos, tablaConceptos, true);
			...
			if (concepto.getImpuestos().getTraslados().getTraslado().size() > 0) {
		...
		tablaConceptos.setSpacingAfter(2F);
