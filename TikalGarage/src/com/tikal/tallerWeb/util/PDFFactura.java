...
public class PDFFactura {
	...
	public Document construirPdf(Comprobante comprobante, String selloDigital, byte[] bytesQRCode, Imagen imagen,
			Estatus estatus) throws DocumentException, MalformedURLException, IOException {
...
//		construirHechoPor();
	...
	public Document construirPdfCancelado(Comprobante comprobante, String selloDigital, byte[] bytesQRCode,
			Imagen imagen, Estatus estatus, String selloCancelacion, Date fechaCancelacion)
	...
	private void construirBoceto(Comprobante comprobante, Imagen imagen, Estatus estatus, TimbreFiscalDigital tfd)
		...
		if (numCtaPago != null) {
			if (!numCtaPago.contentEquals("")) {
				String strNumCtaPago = "N�mero de cuenta de pago: ".concat(numCtaPago);
		...
		boolean[] existeISReIVA = agregarEtiquetaRetenciones(comprobante.getImpuestos().getRetenciones(),
				subTablaEtqTotal);
	...
	public static class MyFooter extends PdfPageEventHelper {
		...
		public void onEndPage(PdfWriter writer, Document document) {
			...
			/*
			 * Phrase footer = new
			 * Phrase("Hoja n�mero ".concat(Integer.toString(document.
